...
/* massG default gives the real coated-stack film: t = m/(ρL²) = 2.60 µm */
const DEFAULTS = { F: 250, g: 9.81, L: 15, massG: 836, tgt: 2.3, E_GPa: 4.4, rho: 1430,
                   phiDeg: 10, slew: 0.83 };
...
/* share the exact current state as a URL (only non-default keys) */
$('btnShare').addEventListener('click', () => {
  const qp = new URLSearchParams();
  for (const [q, key] of Object.entries(QMAP))
    if (state[key] !== DEFAULTS[key]) qp.set(q, state[key]);
  if (document.documentElement.dataset.theme === 'dark') qp.set('theme', 'dark');
  const url = location.href.split(/[?#]/)[0] + (qp.toString() ? '?' + qp.toString() : '');
  const btn = $('btnShare'), old = btn.textContent;
  const flash = msg => { btn.textContent = msg; setTimeout(() => { btn.textContent = old; }, 1500); };
  if (navigator.clipboard && navigator.clipboard.writeText)
    navigator.clipboard.writeText(url).then(() => flash('copied ✓'), () => flash(url));
  else flash(url);
});
