 ...
 * gravity load. sigGm2 = the same thing per area (g/m²): the two controls are
 * linked (sig = massG/L²), and dragging side length holds DENSITY fixed and
 * rescales mass — so slope vs L behaves like a real build, not a fixed lump.
 * tUm = film thickness, a direct input: it only enters through E·t (in-plane
 * stretch, stiffening, film stress), never the sag shape. */
const DEFAULTS = { F: 250, g: 9.81, L: 15, massG: 1000, sigGm2: 1000 / 225, tgt: 2.3,
                   E_GPa: 4.4, tUm: 2.6, phiDeg: 10, grabPct: 3, slew: 0.83 };
...
const QLIM = { F: [0, 1000], g: [0, 9.81], L: [0.5, 20], massG: [0.1, 20000], tgt: [0.1, 20],
               ...
               slew: [0, 5] };
{
  ...
  state.sigGm2 = state.massG / (state.L * state.L);   // density follows m and L from the URL
}
...
/* mass ↔ density linkage: sig = massG/L² always. Editing either rewrites the
 * other; editing L holds density and rescales mass. */
function reflectDensity() {
  $('rngM').value = state.massG;
  $('numM').value = Math.round(state.massG * 10) / 10;
  $('rngSig').value = state.sigGm2;
  $('numSig').value = Math.round(state.sigGm2 * 100) / 100;
}
const setters = {
  ...
  sigGm2: bindPair('rngSig', 'numSig', 'sigGm2'),
...
function onControl(key, noSnap) {
  ...
  if (key === 'massG' || key === 'sigGm2' || key === 'L') {
    const A = state.L * state.L;
    if (key === 'massG') state.sigGm2 = state.massG / A;
    else state.massG = Math.min(20000, Math.max(0.1, state.sigGm2 * A));
    state.sigGm2 = state.massG / A;                       // re-derive after any clamp
    reflectDensity();
  }
